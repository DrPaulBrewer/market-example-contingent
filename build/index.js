 ...
 */
const orderHeader = ['count', 'tlocal', 't', 'tx', 'id', 'cancel', 'q', 'buyPrice', 'sellPrice', 'buyStop', 'buyStopPrice', 'sellStop', 'sellStopPrice', 'triggersBuyPrice', 'triggersSellPrice', 'triggersBuyStop', 'triggersBuyStopPrice', 'triggersSellStop', 'triggersSellStopPrice'];
/**
...
function ao(ordera) {
  ...
  for (i = offset; i < l; ++i) if (i <= 6 || ordera[i - offset]) obj[orderHeader[i]] = ordera[i - offset];
...
class Market extends _marketEngine.MarketEngine {
  /**
   * Market constructor
   *
  ...
  bookSizeRule() {
    ...
    if (buySellBookLimit > 0) {
      ...
      [this.book.buy, this.book.sell].forEach(function (B) {
        ...
        for (i = 0, l = Math.min(B.idx.length, buySellBookLimit); i < l; ++i) keep[B.idx[i]] = 1;
      });
      [this.book.buyStop, this.book.sellStop].forEach(function (B) {
        let i, l;

        for (i = 0, l = B.idx.length; i < l; i++) keep[B.idx[i]] = 1;
      });
      const keepidx = Object.keys(keep).sort(function (a, b) {
        return +a - b;
      });
      let i,
          l,
      ...
      for (i = 0, l = keepidx.length; i < l; ++i) temp[i] = this.a[keepidx[i]];

      for (i = 0, l = keepidx.length; i < l; ++i) this.a[i] = temp[i];
  ...
  findAndProcessTrades() {
    ...
    while ((seqtrades = _marketPricing.default.sequential(this.book.buy.idxdata(), this.book.sell.idxdata(), this.o.countCol, this.o.bpCol, this.o.qCol, this.o.spCol, this.o.qCol)) !== undefined) {
      ...
      for (i = 0, l = tradeSpec.buyA.length; i < l; ++i) tradeSpec.buyId[i] = this.a[tradeSpec.buyA[i]][this.o.idCol];

      for (i = 0, l = tradeSpec.sellA.length; i < l; ++i) tradeSpec.sellId[i] = this.a[tradeSpec.sellA[i]][this.o.idCol];
  ...
  stopsTrigger(t, matches) {
    ...
    if (matches[0]) {
      ...
      const newOrders = bs.idxdata().slice(0, matches[0]).map(toBuyAtMarket);
    ...
    if (matches[1]) {
      ...
      const newOrders = ss.idxdata().slice(0, matches[1]).map(toSellAtMarket);
  ...
  triggerOrderToInbox(j, q, t) {
    ...
    if (myorder && myorder[qCol] >= q) {
      if (myorder[trigSliceBegin] > 0 || myorder[trigSliceBegin + 1] > 0 || myorder[trigSliceBegin + 2] > 0 || myorder[trigSliceBegin + 3] > 0 || myorder[trigSliceBegin + 4] > 0 || myorder[trigSliceBegin + 5] > 0) {
        ...
        for (let ii = 0, ll = trigSliceEnd - 2; ii < ll; ++ii) trigorder[ii] = 0;

        trigorder[tCol - 2] = t;
        trigorder[idCol - 2] = myorder[idCol];
        trigorder[qCol - 2] = q;

        for (let ii = 0, ll = trigSliceEnd - trigSliceBegin; ii < ll; ++ii) trigorder[bpCol + ii - 2] = myorder[ii + trigSliceBegin];
  ...
  tradeTrigger(tradespec) {
    const t = tradespec.t;
    const buyA = tradespec.buyA,
          sellA = tradespec.sellA;
    const buyQ = tradespec.buyQ,
          sellQ = tradespec.sellQ;
    if (buyA) for (let i = 0, l = buyA.length; i < l; ++i) {
      this.triggerOrderToInbox(buyA[i], buyQ[i], t);
    }
    if (sellA) for (let i = 0, l = sellA.length; i < l; ++i) {
